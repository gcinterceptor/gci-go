...
// check verifies if the current amount of heap consumed processing requests is bigger than the shedding threshold.
func (h *heap) check() bool {
	return (h.rt.HeapAlloc() - h.cleanSlate) >= atomic.LoadUint64(&h.st)
}

// collect performs a garbage collection. It also updates the shedding threshold based on amount of heap
// consumed during request processing since the last GC.
func (h *heap) collect() {
